...
// Adaptations: card.glb loads from /models, strap falls back to a solid
// signal color when no lanyard image is supplied, physics tuned per props.
'use client';
...
function Band({
  ...
  // Rapier body handles are untyped on purpose: the joint hooks and imperative
  // calls below only need the runtime shape, and strict body types add noise.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const fixed: any = useRef(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const j1: any = useRef(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const j2: any = useRef(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const j3: any = useRef(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const { nodes, materials } = useGLTF('/models/card.glb') as unknown as {
    ...
    materials: Record<string, THREE.Material & { map: THREE.Texture | null }>;
  };
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      (band.current?.geometry as unknown as { setPoints: (pts: THREE.Vector3[]) => void } | undefined)?.setPoints(curve.getPoints(isMobile ? 16 : 32));
