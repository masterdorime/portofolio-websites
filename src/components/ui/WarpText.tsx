...
interface WarpPropBag extends WarpTextProps {
  text: string;
  color: string;
  fontSize: string;
  fontWeight: number;
...
const syncUniforms = (program: Program, props: WarpPropBag): void => {
...
const WarpText = ({
  ...
  const propsRef = useRef<WarpPropBag>({
    text,
    color,
    fontSize,
    fontWeight,
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (!contextLost && gl) {
        try {
          const tex = (texture as unknown as { texture?: WebGLTexture | null } | undefined)?.texture;
          if (tex) gl.deleteTexture(tex);
          (geometry as unknown as { remove?: () => void } | undefined)?.remove?.();
          (program as unknown as { remove?: () => void } | undefined)?.remove?.();
          (gl.getExtension('WEBGL_lose_context') as unknown as { loseContext?: () => void } | null)?.loseContext?.();
        } catch (error) {
          void error;
        }
      }
  ...
  return (
    <div ref={containerRef} className={[`warp-text`, className].filter(Boolean).join(' ')} style={style} role="img" aria-label={text} />
  );
};
