...
const DEFAULT_ITEMS: MorphItem[] = [
  { image: 'https://picsum.photos/seed/morph1/1600/1000', caption: 'One' },
  { image: 'https://picsum.photos/seed/morph2/1600/1000', caption: 'Two' },
  { image: 'https://picsum.photos/seed/morph3/1600/1000', caption: 'Three' },
];
...
vec2 coverUV(vec2 uv, vec2 res, vec2 img) {
  ...
  float iA = img.x / max(img.y, 0.0001);
  vec2 s = vec2(1.0);
...
class MorphEngine {
  ...
  constructor(
    ...
    this.sizes = this.items.map((): [number, number] => [1, 1]);
  ...
  resize() {
    ...
    (this.program.uniforms.uResolution.value as Float32Array | number[])[0] = this.gl.canvas.width;
    (this.program.uniforms.uResolution.value as Float32Array | number[])[1] = this.gl.canvas.height;
  }
  ...
  setPointer(x: number, y: number) {
    (this.program.uniforms.uPointer.value as number[])[0] = x;
    (this.program.uniforms.uPointer.value as number[])[1] = y;
  }
  ...
  destroy() {
    ...
    this.textures.forEach(tex => {
      const glTex = (tex as unknown as { texture?: WebGLTexture | null }).texture;
      if (glTex) this.gl.deleteTexture(glTex);
    });
    const prog = this.program as unknown as { program?: WebGLProgram | null };
    if (prog.program) this.gl.deleteProgram(prog.program);
    const ext = this.gl.getExtension('WEBGL_lose_context') as unknown as { loseContext?: () => void } | null;
    ext?.loseContext?.();
...
export default function MorphSlider({
  ...
  const optsRef = useRef<MorphOptions | null>(null);
  optsRef.current = { transition, duration, ease, intensity, scale, aberration, drift, overlayColor, loop };

  useEffect(() => {
    if (!containerRef.current || !optsRef.current) return undefined;
    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const getOptions = () => optsRef.current as MorphOptions;
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (active && el.setPointerCapture) {
        try {
          ...
          // pointer already released — drag simply won't track
        }
      }
    };
  ...
  return (
    <div
      className={[`morph-slider`, className].filter(Boolean).join(' ')}
      style={{
